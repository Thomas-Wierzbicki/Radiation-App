...
// PLZ Filter-Liste (z.B. "0,1,8,9") -> ['0', '1', '8', '9']
...
function buildMappedStations(features) {
    return features.map((f) => {
        ...
        if (FILTER_PLZ_ZONES.length > 0) {
            const firstDigit = (stationPlz && stationPlz.length > 0) ? stationPlz[0] : null;
            if (!FILTER_PLZ_ZONES.includes(firstDigit)) {
                return null; // Überspringen, wenn nicht in der Liste
            }
        }
        // ------------------------
...
async function updateRadiationData() {
    ...
    try {
        ...
        console.log(`✅ ${mappedStations.length} Stationen verarbeitet (Filter aktiv: ${FILTER_PLZ_ZONES.length > 0})`);
...
app.get('/healthz', (req, res) => {
    res.json({
        ...
        cachedEntries: cachedRadiationData.length,
        plzFilter: FILTER_PLZ_ZONES,
...
app.listen(PORT, '0.0.0.0', async () => {
    ...
    if (FILTER_PLZ_ZONES.length > 0) {
        console.log(`🔍 Filter aktiv für PLZ-Zonen: ${FILTER_PLZ_ZONES.join(', ')}`);
    } else {
        console.log(`🌐 Lade alle Regionen (kein PLZ-Filter)`);
    }
