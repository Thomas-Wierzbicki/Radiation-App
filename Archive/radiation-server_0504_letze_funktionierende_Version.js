...
// Schwelle für "rot"
const RED_THRESHOLD = parseFloat(process.env.RED_THRESHOLD || '0.14');
...
function publishRedStation(station) {
    if (!mqttConnected) {
        console.warn(`MQTT nicht verbunden, überspringe: ${station.n || 'unbekannt'}`);
        return;
    }
    ...
    // Nur ASCII, keine Emojis, kein µ, keine problematischen Zeichen
    ...
    mqttClient.publish(MQTT_TOPIC, payloadStr, (err) => {
        if (err) {
            console.error(`MQTT publish Fehler fuer ${place}:`, err.message);
        } else {
...
function buildMappedStations(features) {
    return features.map((f) => {
        ...
        // Nur verwerfen, wenn gar keine brauchbaren Koordinaten vorhanden sind
        if (!coords) {
            return null;
        }
...
function logRedStations(prefix, stations) {
    ...
    if (redStations.length > 0) {
        redStations.forEach((s) => {
            console.log(`   • ${s.n} | ${s.v} µSv/h | ${s.plz || 'ohne PLZ'} | ${s.coord_source}`);
        });
    }
...
app.get('/api/radiation', async (req, res) => {
    ...
    if (cachedRadiationData && (now - lastCacheTime < CACHE_DURATION)) {
        ...
        const redStations = logRedStations('Cache', cachedRadiationData);
    ...
    console.log('🌐 CACHE MISS - lade Daten vom BfS');
    ...
    try {
        ...
        const maxValue = mappedStations.length
            ...
            : 0;
        console.log(`📈 Maximalwert: ${maxValue} µSv/h`);

        cachedRadiationData = mappedStations;
        lastCacheTime = now;

        const redStations = logRedStations('Live', mappedStations);

        // Frontend sofort bedienen
        res.json(mappedStations);

        // MQTT danach, damit Frontend nicht blockiert
        sendRedStationsAsync(redStations);
