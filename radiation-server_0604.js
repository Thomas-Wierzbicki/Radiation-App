...
async function updateRadiationData(bundeslandCodes = []) {
    if (updateRunning) {
    ...
    try {
        // 1. Anfrage ohne Server-Filter vorbereiten (stabilste Methode)
        const queryParams = {
            service: 'WFS',
            version: '1.1.0',
            request: 'GetFeature',
        ...
        // 2. Daten vom BfS abrufen
        const response = await axios.get('https://bfs.de', {
            params: queryParams,
            timeout: 45000
        });

        // 3. Basis-Check der Antwort
        if (!response.data || !Array.isArray(response.data.features)) {
            console.error('❌ Unerwartete Antwort vom BfS (Keine Features)');
            return;
        }

        let features = response.data.features;

        // 4. LOKALER FILTER (Hier filtern wir die Liste in deinem RAM)
        if (Array.isArray(bundeslandCodes) && bundeslandCodes.length > 0) {
            const codes = bundeslandCodes.map(c => c.toUpperCase());
            features = features.filter(f => {
                const bl = f.properties?.kenn_bl;
                return bl && codes.includes(bl.toUpperCase());
            });
            console.log(`🔍 Lokal gefiltert auf: ${codes.join(', ')} (${features.length} Stationen gefunden)`);
        }

        // 5. Mappen der (gefilterten) Stationen
        const mappedStations = buildMappedStations(features);
        ...
        const maxValue = mappedStations.length
            ...
            : 0;
        console.log(`📈 Maximalwert: ${maxValue} uSv/h | Quelle: BfS=${bfsCount}, PLZ=${plzCount}`);
